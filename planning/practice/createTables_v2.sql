...
CREATE TABLE IF NOT EXISTS ticket (
    ...
    --included race_id so tickets are directly linked to races
    --Accounts for the case when a horse participates in more than one race.
    --Avoids the need for joining horse table for ticket information queries.
    --Might not be needed if its an one time thing for each horse
    race_id INTEGER NOT NULL,
